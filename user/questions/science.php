...
if (isset($_POST['attemptid']) && isset($_POST['mark'])) {
    ...
    $sql = $conn->prepare("SELECT * FROM markc WHERE attemptid =:attemptid");
    ...
    if ($result > 0) {
        ...
        $sql = $conn->prepare("INSERT INTO markc (attemptid, mark) VALUES (:attemptid, :mark)");
        $sql->execute(array(
...
<body>
  ...
  <div class="header">
    <h1>Question : Science</h1>
  </div>

  <div class="left">

    <?php
  $sql = $conn->query("SELECT * FROM questionc order by qnum");

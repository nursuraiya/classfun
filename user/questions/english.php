...
if (isset($_POST['attemptid']) && isset($_POST['mark'])) {
    ...
    $sql = $conn->prepare("SELECT * FROM markd WHERE attemptid =:attemptid");
    ...
    if ($result > 0) {
        ...
        $sql = $conn->prepare("INSERT INTO markd (attemptid, mark) VALUES (:attemptid, :mark)");
        $sql->execute(array(
...
<body>
  ...
  <div class="header">
    <h1>Question : English</h1>
  </div>

  <div class="left">
    <?php
  $sql = $conn->query("SELECT * FROM questiond order by qnum");

...
if (isset($_POST['attemptid']) && isset($_POST['mark'])) {
    ...
    $sql = $conn->prepare("SELECT * FROM marka WHERE attemptid =:attemptid");
    ...
    if ($result > 0) {
        ...
        $sql = $conn->prepare("INSERT INTO marka (attemptid, mark) VALUES (:attemptid, :mark)");
        $sql->execute(array(
...
<body>
  ...
  <div class="header">
    <h1>Question : Bahasa Melayu</h1>
  </div>

  <div class="left">

    <?php
$sql = $conn->query("SELECT * FROM questiona order by qnum");
